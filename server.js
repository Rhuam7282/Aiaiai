...
app.use(
  cors({
    origin: [
      ...
      "http://127.0.0.1:5500/",
      "https://aiaiai-ibk2.onrender.com/",
    ], // Permite requisições apenas do seu frontend no Netlify
...
if (!API_KEY) {
  console.error(
    "!!! Atenção: GEMINI_API_KEY não definida nas variáveis de ambiente da plataforma !!!"
  );
  process.exit(1);
} else {
...
async function initializeDatabases() {
  ...
  if (!dbLogs && !dbHistoria) {
    console.error(
      "Falha ao conectar a qualquer banco de dados. Verifique as URIs e configurações."
    );
  }
}
...
async function getTimezoneInfo(city) {
  try {
    const response = await fetch(
      `https://api.api-ninjas.com/v1/timezone?city=${encodeURIComponent(city)}`,
      {
        headers: {
...
function detectWeatherOrTimeQuery(message) {
  ...
  const cityMatch = lowerMessage.match(
    /(?:em|de|do|da|no|na|para)\s+([a-zà-ú\s]+)(?:\?|\.|\!|$)/
  );
  if (cityMatch && cityMatch[1]) {
    city = cityMatch[1].trim();
    // Remover palavras-chave de clima/tempo do nome da cidade, se houver
    weatherKeywords.forEach(
      (kw) => (city = city.replace(new RegExp(`\b${kw}\b`, "g"), "").trim())
    );
    timeKeywords.forEach(
      (kw) => (city = city.replace(new RegExp(`\b${kw}\b`, "g"), "").trim())
    );
  }
  ...
  if (!city || city.length < 2) {
    // Considerar cidades com pelo menos 2 caracteres
...
Vento: ${weatherData.vento} m/s\n`;
    ...
    if (timeData) {
      contextInfo += `\n\nDados de Horário para ${
        timeData.cidade || "a cidade solicitada"
      }:
...
Offset UTC: ${timeData.offset_utc}h\n`;
    ...
    const fullPrompt =
      ...
      chatHistory
        .map(
          (msg) =>
            `${msg.role === "user" ? "Humano" : "DIO"}: ${msg.parts[0].text}`
        )
        .join("\n") +
...
app.post("/api/log-connection", async (req, res) => {
  if (!dbLogs) {
    return res
      .status(500)
      .json({ error: "Servidor não conectado ao banco de dados de logs." });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Erro interno ao registrar log de conexão." });
  }
});
...
app.post("/api/chat/salvar-historico", async (req, res) => {
  if (!dbHistoria) {
    return res.status(500).json({
      error: "Servidor não conectado ao banco de dados de histórico.",
    });
  }
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Dados incompletos para salvar histórico (sessionId, botId, messages são obrigatórios).",
      });
    }
    ...
    console.error(
      "[Servidor] Erro em /api/chat/salvar-historico:",
      error.message
    );
    res
      .status(500)
      .json({ error: "Erro interno ao salvar histórico de chat." });
  }
});
  ...
  try {
    ...
    const historicosFormatados = historicos.map((hist) => ({
      ...
      preview: hist.preview
        ? hist.preview.parts[0].text.substring(0, 50) + "..."
...
app.get("/api/chat/historico/:sessionId", async (req, res) => {
  ...
  try {
    ...
    console.error(
      "[Servidor] Erro em /api/chat/historico/:sessionId:",
      error.message
    );
    res
      .status(500)
      .json({ error: "Erro interno ao buscar sessão específica." });
  }
});
...
app.get("/api/test", (req, res) => {
  res.json({
    message:
      "MUDA MUDA MUDA! O servidor do Dio-sama está funcionando perfeitamente!",
    timestamp: new Date().toISOString(),
    databases: {
